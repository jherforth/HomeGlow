...
(`{ userId, amount, newTotal }`), `chore.completed` / `chore.uncompleted`
(`{ userId, choreId, scheduleId, clamValue, date }`), `chore.allCompleted`
(`{ userId, username, date, reward }` — a user finished every regular chore for
the day), and `prize.redeemed`. Only events named in your
